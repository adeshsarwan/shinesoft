	...
	public class IAPManager : SingletonComponent<IAPManager>, ISaveable
	{
		...
		public void OnInitializeFailed(InitializationFailureReason error, string message)
		{
			Logger.LogError(LogTag,
				$"Initialization failed! Error: {error}, Message: {message}");
		}

		public void OnPurchaseFailed(Product product, PurchaseFailureReason failureReason)
		{
			Logger.LogError(LogTag,
				$"Purchase failed. Product: {product.definition.id}, Reason: {failureReason}");
		}
		...
		public void RestorePurchases()
		{
			...
			if (IsInitialized)
			{
				     ...
				     Application.platform == RuntimePlatform.OSXPlayer))
				{
					...
					{
						Logger.Log(LogTag, $"Restore Success: {success}, Error: {error}");
					});
				}
				else
				{
