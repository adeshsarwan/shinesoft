...
public final class ReelsAdapter extends RecyclerView.Adapter<ReelsAdapter.ReelViewHolder> {
    ...
    private static ReelsAdapter instance;
    ...
    int episodeIndex;
    public ReelsAdapter(Context context, ReelsFragment reelsFragment, List<EpisodeListModel.Datum> reels, boolean z, boolean z2, ActivityResultLauncher<Intent> infoLauncher,  int episodeIndex) {
        ...
        this.updateSeekBarRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (ReelsAdapter.this.getCurrentPlayingPosition() != -1 && ReelsAdapter.this.getCurrentPlayingPosition() < ReelsAdapter.this.players.size() && (exoPlayer = (ExoPlayer) ReelsAdapter.this.players.get(ReelsAdapter.this.getCurrentPlayingPosition())) != null) {
                    ReelsAdapter seriesReelsAdapter = ReelsAdapter.this;
                ...
                ReelsAdapter.this.handler.postDelayed(this, 500L);
            }
        };
    ...
    public static final class Companion {
        ...
        public final ReelsAdapter getInstance() {
            return ReelsAdapter.instance;
        }

        public final void setInstance(ReelsAdapter seriesReelsAdapter) {
            ReelsAdapter.instance = seriesReelsAdapter;
        }
    }
...
public void onViewRecycled(@NonNull ReelViewHolder holder) {
    ...
    public final class ReelViewHolder extends RecyclerView.ViewHolder {
        ...
        final ReelsAdapter fullReelsAdapter;

        public ReelViewHolder(ReelsAdapter seriesReelsAdapter, ItemReelBinding binding) {
        ...
        private void saveContinueWatching(
                EpisodeListModel.Datum episode,
                long positionMs,
                long durationMs,
                boolean b) {
            ...
            map.put("seriesId", episode.getId());
            map.put("lastPositionMs", positionMs);
            map.put("completionPct", completionPct);
            map.put("status", status);
            map.put("lastWatchedAt", FieldValue.serverTimestamp());

            map.put("seriesTitle", episode.getTitle());
            map.put("episodesIndex", episode.getEpisodePart().getPartIndex());
            map.put("episodeTitle",episode.getTitle());
             map.put("seriesThumbnail", episode.getCover());
             map.put("episodesTotalCount",episode.getEpisodeCount());
             map.put("seriesThumbnail", episode.getCover());
        ...
        private void saveEpisodeProgress(
            ...
            Map<String, Object> map = new HashMap<>();
            map.put("episodeId", episode.getEpisodePart().getEpisodeId());
            map.put("seriesId", episode.getId());
            map.put("durationMs", durationMs);
            map.put("lastPositionMs", positionMs);
            map.put("completionPct", completionPct);
            map.put("status", status);
            map.put("episode_number", episode.getEpisodePart().getPartIndex());
            map.put("lastWatchedAt", FieldValue.serverTimestamp());
        ...
        private void saveProgressIfNeeded(
            ...
            saveEpisodeProgress(episode, position, duration);
            saveContinueWatching(episode, position, duration,false);
        }
        ...
        public final void bind(ReelViewHolder holder, final EpisodeListModel.Datum reelInfo, boolean z, boolean z2, final int i) {
            ...
            getBinding().linearWatchFullSeries.setVisibility(VISIBLE);
            ...
            binding.playTotal.setText(" /EP. " + reels.get(i).getEpisodeCount());
            binding.playCur.setText("EP." + reels.get(i).getEpisodePart().getPartIndex());
            context.getSharedPreferences("EpisodePrefs", Context.MODE_PRIVATE)
                    .edit()
                    .putInt("selected_episode", reelInfo.getEpisodePart().getPartIndex().intValue() - 1)
                    .apply();
            binding.linearWatchFullSeries.setVisibility(VISIBLE);
            ...
            this.binding.imgShare.setOnClickListener(new View.OnClickListener() {
                ...
                public final void onClick(View view) {
                    if (reelInfo == null || reelInfo.getCover() == null || reelInfo.getTitle() == null) return;
            ...
            getBinding().relativeLikeButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    boolean newState = !reels.get(i).isFavourite();
                    reels.get(i).setFavourite(newState);
                    ...
                    build.addListener(new Player.Listener() {
                        ...
                        public void onPlaybackStateChanged(int playbackState) {
                            if (playbackState == Player.STATE_READY) {
                                ...
                                // If this is the current playing position and should auto-play, start playing
                                if (shouldAutoPlay && getAdapterPosition() == ReelsAdapter.this.getCurrentPlayingPosition()) {
                                    build.setPlayWhenReady(true);
                                    build.play();
                                }
                            } else if (playbackState == Player.STATE_BUFFERING && shouldAutoPlay
                                    && getAdapterPosition() == ReelsAdapter.this.getCurrentPlayingPosition()) {
                                // Ensure playWhenReady is set during buffering
                                build.setPlayWhenReady(true);
                            }
                        }
                        ...
                        public void onPlayWhenReadyChanged(boolean playWhenReady, int reason) {
                            if (playWhenReady && getAdapterPosition() == ReelsAdapter.this.getCurrentPlayingPosition()) {
                                if (build.isPlaying()) {
            ...
            // Show ad after every 'N' reels as defined in preferences and only if ads are enabled
            int adFrequency = ControlPreference.getExoplayerImaAfterReelsShowCount();
            // Debug logs for ad eligibility and position
//            Log.d("IMA_AD", "Processing reel at adapter position=" + i + ", adFrequency=" + adFrequency);
            boolean isImaEnabled = ControlPreference.getExoplayerImaShow()
                    && !com.snapdrama.shortstream.ads.PremiumPlanManager.shouldSkipAfterLoginAd(context);
//            Log.d("IMA_AD", "IMA enabled=" + isImaEnabled);
            // Show ad after every 'adFrequency' reels (1‑based index)
            boolean shouldShowImaAd = (i >= 0) && (adFrequency > 0) && ((i + 1) % adFrequency == 0) && isImaEnabled;
//            Log.d("IMA_AD", "shouldShowImaAd=" + shouldShowImaAd + " for position=" + i);
            if (!shouldShowImaAd) {
//                Log.d("IMA_AD", "Ad NOT shown for position=" + i);
            }
            MediaSource.Factory mediaSourceFactory;
            DataSource.Factory dataSourceFactory;
            if (shouldShowImaAd) {
                // Always create a fresh ImaAdsLoader for each ad block to avoid stale state
                if (fullReelsAdapter.adsLoader != null) {
                    fullReelsAdapter.adsLoader.release();
                }
                fullReelsAdapter.adsLoader = new ImaAdsLoader.Builder(context)
                        .setAdEventListener(fullReelsAdapter.buildAdEventListener())
                        .setImaSdkSettings(fullReelsAdapter.getImaSdkSettings())
                        .build();
                ...
                mediaSourceFactory = new DefaultMediaSourceFactory(dataSourceFactory)
                        .setLocalAdInsertionComponents(
                                unusedAdTagUri -> fullReelsAdapter.adsLoader,
                                binding.playerView
                        );
            } else {
                mediaSourceFactory = new DefaultMediaSourceFactory(new DefaultDataSource.Factory(context));
            }

            releasePlayer();
            build = new ExoPlayer.Builder(this.fullReelsAdapter.context).setMediaSourceFactory(mediaSourceFactory).build();
            final ReelsAdapter seriesReelsAdapter = this.fullReelsAdapter;
...
public void onPlayWhenReadyChanged(boolean playWhenReady, int reason) {
    ...
    if (playWhenReady) {
        ...
        getBinding().playPauseIcon.setImageResource(R.drawable.ic_play);
        saveProgressIfNeeded(build, reelInfo);
    }
}
                });
            }
            ...
            if (shouldShowImaAd && fullReelsAdapter.adsLoader != null) {
                fullReelsAdapter.adsLoader.setPlayer(build);
            }
...
//            this.binding.btnBack.setVisibility(View.INVISIBLE);
            this.binding.textSeriesName.setText(reels.get(i).getTitle());
            this.binding.textSeriesDescription.setText(reels.get(i).getDescription());
...
//            });
            ...
            final ReelsAdapter seriesReelsAdapter3 = this.fullReelsAdapter;
...
//                    seriesReelsAdapter.onMuteChangeListener.invoke(Boolean.valueOf(seriesReelsAdapter.isMuted));
            ...
            boolean isAdded = addedEpisodeIds.contains(reels.get(i).getId());
            ...
            binding.linearAddToList.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    boolean alreadyAdded = addedEpisodeIds.contains(reelInfo.getId());

                    if (alreadyAdded) {
                        removeFromList(reelInfo.getId());
                        addedEpisodeIds.remove(reelInfo.getId());
                        binding.imageBookMark.setImageResource(R.drawable.image_book_mark_icon);
                        binding.textAddList.setText(context.getString(R.string.add_list));
                    } else {
                        saveAddListData(reelInfo, true);
                        addedEpisodeIds.add(reelInfo.getId());
                        binding.imageBookMark.setImageResource(R.drawable.image_book_mark_icon);
                        binding.textAddList.setText(context.getString(R.string.added_1));
        ...
        private void saveAddListData(EpisodeListModel.Datum series, boolean added) {
        ...
        private void removeFromList(String series) {
        ...
        public final void releasePlayer() {
            try {
                ...
                if (exoPlayer != null) {
                    ...
                    ReelsAdapter seriesReelsAdapter = this.fullReelsAdapter;
                    exoPlayer.stop();
                    exoPlayer.release();
...
textViewSpeed.setText(String.valueOf(v)+"x");
    }
